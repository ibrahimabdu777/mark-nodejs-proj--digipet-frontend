...
function App() {
  ...
  const loadDataFromEndpoint = async (endpoint: `/${string}`) => {
    ...
    try {
      const res = await fetch(`https://enigmatic-citadel-80124.herokuapp.com${endpoint}`);
    ...
    <main>
      ...
      <DigipetActions
        actions={[
          ...
          { name: "Feed", handler: () => loadDataFromEndpoint("/digipet/feed") },
          {name: "Train", handler: () => loadDataFromEndpoint("/digipet/train")},
          {name: "Rehouse", handler: () => loadDataFromEndpoint("/digipet/rehouse")}
        ]}
      />
    </main>
  );
